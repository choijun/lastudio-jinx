...
abstract class LaStudioKit_Base extends Widget_Base
{
    ...
    public function render_masonry_filters($container = 'parent', $echo = true)
    {
        ...
        if ($enable_filter && !empty($filters)) {
            $output .= '<div class="lakit-masonry_filter" data-lakitmasonry_container="'.esc_attr($container).'">';
            ...
            foreach ($filters as $filter_id) {
                ...
                if(!is_wp_error($termObj)){
                    $output .= sprintf('<div class="lakit-masonry_filter-item" data-filter="term-%1$s">%2$s</div>', esc_attr($filter_id), $termObj->name);
                }
            }
            $output .= '</div>';
            $output .= '</div>';
